...
module.exports = async (client) => {
   ...
   setInterval(function Activitys(){
      ...
      let Presence = [ "dnd", "online", "idle" ]; //can be: online | dnd | idle | offline
      let PresencePower = Presence[Math.floor(Math.random() * Presence.length)]
      let Activity = [
        `${prefix}help`,
        `${prefix}nuck`
      ];
      let ActivityPower = `${Activity[Math.floor(Math.random() * Activity.length)]} | Nucker Bot`;
      let Display = [ 0, 1, 2, 3, 5 ]; //can be: Discord.ActivityType.Competing = 5 or	Discord.ActivityType.Custom =	4 or Discord.ActivityType.Listening	= 2 or Discord.ActivityType.Playing =	0 or Discord.ActivityType.Streaming =	1 or Discord.ActivityType.Watching =	3
